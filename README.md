...
Cursor MCP server that clones [wordpress-docker-tailscale](https://github.com/liquidloose/wordpress-docker-tailscale) into a numbered site folder, writes `.env` from `.env.sample`, and overlays default editor settings.
...
2. **`bootstrap_site`** — Required `slug` and `magicdns_hostname`. Optional `content_repo_url`, `web_port`, `phpmyadmin_port`, `start_compose` (default `false`).
